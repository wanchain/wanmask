...
function detectAndRunEntryTask () {
  ...
  if (!taskName) {
    throw new Error(`WanMask build: No task name specified`)
  }
...
async function runTask (taskName, { skipStats } = {}) {
  if (!(taskName in tasks)) {
    throw new Error(`WanMask build: Unrecognized task name "${taskName}"`)
  }
  if (!skipStats) {
  ...
  try {
    ...
    console.error(`WanMask build: Encountered an error while running task "${taskName}".`)
    console.error(err)
    process.exit(1)
  }
...
function createTask (taskName, taskFn) {
  if (taskName in tasks) {
    throw new Error(`WanMask build: task "${taskName}" already exists. Refusing to redefine`)
  }
...
function runInChildProcess (task) {
  ...
  if (!taskName) {
    throw new Error(`WanMask build: runInChildProcess unable to identify task name`)
  }
  return instrumentForTaskStats(taskName, async () => {
    ...
    await new Promise((resolve, reject) => {
      childProcess.once('close', (errCode) => {
        if (errCode !== 0) {
          reject(new Error(`WanMask build: runInChildProcess for task "${taskName}" encountered an error`))
          return
        }
        resolve()
      })
