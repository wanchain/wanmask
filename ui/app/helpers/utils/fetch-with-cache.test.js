...
describe('Fetch with cache', function () {
  ...
  it('fetches a url', async function () {
    nock('https://fetchwithcache.wanmask.io')
      .get('/price')
      .reply(200, '{"average": 1}')

    const response = await fetchWithCache('https://fetchwithcache.wanmask.io/price')
  ...
  it('returns cached response', async function () {
    ...
    fakeLocalStorageHelpers.loadLocalStorageData.returns({
      'https://fetchwithcache.wanmask.io/price': {
  ...
  it('fetches URL again after cache refresh time has passed', async function () {
    ...
    fakeLocalStorageHelpers.loadLocalStorageData.returns({
      'https://fetchwithcache.wanmask.io/cached': {
        cachedResponse: { average: 1 },
        cachedTime: Date.now() - 1000,
      },
    })

    const response = await fetchWithCache('https://fetchwithcache.wanmask.io/price', {}, { cacheRefreshTime: 123 })
    assert.deepEqual(response, {
      average: 3,
    })
  })
  ...
  it('should abort the request when the custom timeout is hit', async function () {
    ...
    await assert.rejects(
      () => fetchWithCache('https://fetchwithcache.wanmask.io/price', {}, { timeout: 20 }),
  ...
  it('throws when the response is unsuccessful', async function () {
    ...
    await assert.rejects(
      () => fetchWithCache('https://fetchwithcache.wanmask.io/price'),
    )
  })
  ...
  it('throws when a POST request is attempted', async function () {
    ...
    await assert.rejects(
      () => fetchWithCache('https://fetchwithcache.wanmask.io/price', { method: 'POST' }),
    )
  })
  ...
  it('throws when the request has a truthy body', async function () {
    ...
    await assert.rejects(
      () => fetchWithCache('https://fetchwithcache.wanmask.io/price', { body: 1 }),
    )
  })
  ...
  it('throws when the request has an invalid Content-Type header', async function () {
    ...
    await assert.rejects(
      () => fetchWithCache('https://fetchwithcache.wanmask.io/price', { headers: { 'Content-Type': 'text/plain' } }),
