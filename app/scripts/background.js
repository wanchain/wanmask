 ...
 * @property {string} provider.type - An identifier for the type of network selected, allows WanMask to use custom provider strategies for known networks.
 ...
 * @property {MetaMaskState} data - The data emitted from WanMask controller, or used to initialize it.
 * @property {Number} version - The latest migration version that has been run.
 */

/**
 * Initializes the WanMask controller, and sets up all platform configuration.
...
async function initialize () {
  ...
  log.debug('WanMask initialization complete.')
}
 ...
 * @returns {Promise<MetaMaskState>} - Last data emitted from previous instance of WanMask.
 */
async function loadStateFromPersistence () {
  ...
  if (versionedData && !versionedData.data) {
    ...
    sentry.captureMessage('WanMask - Empty vault found - unable to recover')
  }
  ...
  if (!versionedData) {
    throw new Error('WanMask - migrator returned undefined')
  }
  ...
  if (localStore.isSupported) {
    ...
    setTimeout(() => {
      throw new Error('WanMask - Localstore not supported')
    })
  }
...
/**
 * Initializes the WanMask Controller with any initial state and default language.
...
function setupController (initState, initLangCode) {
  ...
  pump(
    ...
    (error) => {
      log.error('WanMask - Persistence pipeline failed', error)
    },
  )
  ...
  async function persistData (state) {
    if (!state) {
      throw new Error('WanMask - updated state is missing')
    }
    if (!state.data) {
      throw new Error('WanMask - updated state does not have data')
    }
    if (localStore.isSupported) {
      try {
  ...
  /**
   * Connects a Port to the WanMask controller via a multiplexed duplex stream.
   * This method identifies trusted (WanMask) interfaces, and connects them differently from untrusted (web pages).
...
// On first install, open a new tab with WanMask
