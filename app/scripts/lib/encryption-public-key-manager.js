 ...
 * approved.
 * @property {Object} msgParams.metamaskId Added to msgParams for tracking and identification within WanMask.
...
export default class EncryptionPublicKeyManager extends EventEmitter {
  ...
  addUnapprovedMessageAsync (address, req) {
    return new Promise((resolve, reject) => {
      if (!address) {
        return reject(new Error('WanMask Message: address field is required.'))
      }
      ...
      this.once(`${msgId}:finished`, (data) => {
        switch (data.status) {
          ...
          case 'rejected':
            return reject(ethErrors.provider.userRejectedRequest('WanMask EncryptionPublicKey: User denied message EncryptionPublicKey.'))
          default:
            return reject(new Error(`WanMask EncryptionPublicKey: Unknown problem: ${JSON.stringify(address)}`))
        }
      })
    })
  }
   ...
   *
   * @param {Object} msgParams The msgParams to be used when eth_getEncryptionPublicKey is called, plus data added by WanMask.
   * @param {Object} msgParams.metamaskId Added to msgParams for tracking and identification within WanMask.
