...
export default class DecryptMessageManager extends EventEmitter {
  ...
  addUnapprovedMessageAsync (msgParams, req) {
    return new Promise((resolve, reject) => {
      if (!msgParams.from) {
        return reject(new Error('WanMask Decryption: from field is required.'))
      }
      ...
      this.once(`${msgId}:finished`, (data) => {
        switch (data.status) {
          ...
          case 'rejected':
            return reject(ethErrors.provider.userRejectedRequest('WanMask Decryption: User denied message decryption.'))
          case 'errored':
            return reject(new Error('This message cannot be decrypted'))
          default:
            return reject(new Error(`WanMask Decryption: Unknown problem: ${JSON.stringify(msgParams)}`))
        }
      })
    })
  }
   ...
   *
   * @param {Object} msgParams The msgParams to be used when eth_decryptMsg is called, plus data added by WanMask.
